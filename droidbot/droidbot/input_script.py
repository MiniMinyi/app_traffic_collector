...
class ViewSelector(object):
    ...
    def match(self, view_dict):
        ...
        if 'text' in view_dict and 'resource_id' in view_dict \
            ...
            return False
        if self.text_re and (not view_dict['text'] or not self.text_re.match(view_dict['text'])):
            return False
        if self.resource_id_re and (not view_dict['resource_id'] or not self.resource_id_re.match(view_dict['resource_id'])):
            return False
        if self.class_re and (not view_dict['class'] or not self.class_re.match(view_dict['class'])):
            return False
